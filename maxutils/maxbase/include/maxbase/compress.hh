...
/*
 *  The status INIT_ERROR is set if StreamCompressor or StreamDecompressor
 *  constructors fail to initialize the compression library.
 *  When the status is COMP_ERROR last_comp_error() and last_comp_error_str()
...
/**
 * @brief StreamCompressor class. One instance per thread, decompress can be
...
public:
    /**
     * @brief StreamCompressor  Check the status for INIT_ERROR after creation.
    ...
    /**
     * @brief  If the status() is COMP_ERROR, last_comp_error() and last_comp_error_str()
     ...
     * @return StreamStatus - possible values OK, COMP_ERROR and EMPTY_INPUT_STREAM
     */
...
public:
    /**
     * @brief StreamDecompressor  Check the status for INIT_ERROR after creation.
     *        called multiple times on the same instance (avoid re-creation of
     *        stream context).
     *
    ...
    /**
     * @brief  status - If the status() is COMP_ERROR, last_comp_error() and last_comp_error_str()
     *                  report the underlying compression library error.
     * @return StreamStatus
     */
    CompressionStatus status() const;
