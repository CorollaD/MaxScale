...
//
// The implementation uses a std::variant to share the storage used by the heap and the stack storage
// implementations. This makes it possible to store some values "for free" without having to allocate separate
// space for them. The container might store more values on the stack than is requested if it does not cause
...
public:
    ...
    void push_back(T t)
    {
        if (using_stack())
        {
            Stack& stack = std::get<Stack>(m_storage);
        ...
        {
            std::get<Heap>(m_storage).push_back(std::move(t));
        }
    }
    ...
    void erase(const_iterator it)
    {
        ...
        {
            Heap& heap = std::get<Heap>(m_storage);
    ...
    {
        return using_stack() ? std::get<Stack>(m_storage).data.data() :  std::get<Heap>(m_storage).data();
    }
    ...
    const_iterator end() const
    {
        if (using_stack())
        {
            const Stack& stack = std::get<Stack>(m_storage);
            return stack.data.data() + stack.size;
        }
        else
        {
            const Heap& heap = std::get<Heap>(m_storage);
    ...
    {
        return using_stack() ? std::get<Stack>(m_storage).data.front() : std::get<Heap>(m_storage).front();
    }
    ...
    const_reference back() const
    {
        ...
        {
            return std::get<Heap>(m_storage).back();
        }
    }
    ...
    {
        return using_stack() ? std::get<Stack>(m_storage).size : std::get<Heap>(m_storage).size();
    }

    bool empty() const
    {
        return using_stack() ? std::get<Stack>(m_storage).size == 0 : std::get<Heap>(m_storage).empty();
    }
...
private:
    ...
    {
        return std::holds_alternative<Stack>(m_storage);
    }

    std::variant<Stack, Heap> m_storage{Stack{}};
};
}
