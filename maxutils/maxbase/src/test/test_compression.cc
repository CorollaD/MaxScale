...
{
    if (s.status() == maxbase::StreamStatus::OK)
    {
...
void test_compress(const TestFile& input, const TestFile& compressed)
{
    ...
    maxbase::StreamCompressor compressor(COMPRESSION_LEVEL);
...
void test_decompress(const TestFile& compressed, const TestFile& decompressed)
{
    ...
    maxbase::StreamDecompressor decompressor;
...
{
    maxbase::StopWatch sw;
    using SD = maxbase::StreamDecompressor;
